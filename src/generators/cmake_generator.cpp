...
std::string CMakeGenerator::map_cpp_standard(const std::string& std_str) {
    if (std_str == "stdcpp14" || std_str == "14") return "14";
    if (std_str == "stdcpp17" || std_str == "17") return "17";
    if (std_str == "stdcpp20" || std_str == "20") return "20";
    if (std_str == "stdcpp23" || std_str == "23") return "23";
    if (std_str == "stdcpplatest") return "23";
    return "17"; // Default
}

// Map C standard string to CMake standard number
std::string CMakeGenerator::map_c_standard(const std::string& std_str) {
    if (std_str == "89" || std_str == "90") return "90";
    if (std_str == "99") return "99";
    if (std_str == "11") return "11";
    if (std_str == "17") return "17";
    if (std_str == "23") return "23";
    return "17"; // Default
}
...
void CMakeGenerator::write_target_properties(std::ostream& out, const Project& project, const Solution& solution) {
    ...
    if (detected_lang == "C") {
        std::string c_std = project.c_standard.empty() ? "17" : map_c_standard(project.c_standard);
        props.push_back("C_STANDARD " + c_std);
        ...
        for (const auto& [key, config] : project.configurations) {
            if (!config.cl_compile.language_standard.empty()) {
                cpp_std = map_cpp_standard(config.cl_compile.language_standard);
                break;
            }
        }
